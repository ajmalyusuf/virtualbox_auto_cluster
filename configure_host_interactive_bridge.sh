...
THIS_MAC_ADDRESS="0800271A77F1"
RESTART_PROMPT="ON"

configured_ip_prefix="192.168.1."

if [ "$CLUSTER_NUMBER" == "UNKNOWN" ]
then
	read -p 'Enter the cluster number (For ex: enter '2' for 'c02'): ' cluster_num
	cluster_name="c$(printf %02d $cluster_num)"
else
	cluster_name="c$(printf %02d $CLUSTER_NUMBER)"
fi
...
do
        ...
        host_name="n${counter}-${cluster_name}-${last_part}"
        if [ "$ip" == "$this_ip" ]; then
                this_host_name="${host_name}.hdp.com"
        fi
        mapping="${ip} ${host_name}.hdp.com ${host_name}"
        echo $mapping
...
# -----commented out for bridge network-----
...
grep -iv "IPADDR=" /etc/sysconfig/network-scripts/ifcfg-eth0 > ./ifcfg-eth0.temp
echo "Backing up /etc/sysconfig/network-scripts/ifcfg-eth0..."
echo "Configuring ${this_ip} for eth0 network..."
echo "IPADDR=${this_ip}" >> ./ifcfg-eth0.temp
